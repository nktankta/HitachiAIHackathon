...
# https://qiita.com/sitar-harmonics/items/ac584f99043574670cf3より引用2
...
def findSquares(bin_image, image, cond_area = 1000):
    # 輪郭取得
    label,contours,hie = cv2.findContours(bin_image, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

    for i, cnt in enumerate(contours):
        # 輪郭の周囲に比例する精度で輪郭を近似する
        arclen = cv2.arcLength(cnt, True)
        approx = cv2.approxPolyDP(cnt, arclen*0.05, True)
...
def main():
    image = cv2.imread('predict/0032.png', cv2.IMREAD_COLOR)
    ...
    cv2.imshow('Square Detector', bw)
    cv2.waitKey()
    rimage = findSquares(bw, image,cond_area=100)
